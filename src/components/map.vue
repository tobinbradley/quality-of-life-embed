  ...
  methods: {
    ...
    initMapEvents: function () {
      ...
      map.on('mousemove', 'neighborhoods-fill-extrude', function (e) {
        ...
        let val = prettyNumber(
          ...
          _this.sharedState.metric.config.prefix,
          _this.sharedState.metric.config.suffix
        );
    ...
    getHeight: function () {
      ...
      let heightAdjust = scaleLinear()
        .domain([
          _this.sharedState.breaks[0],
          _this.sharedState.breaks[this.sharedState.breaks.length - 1]
        ])
        .range([0, 3000]);
